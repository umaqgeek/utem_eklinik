 ...
 *
 * @author Hadi Akmal
 * call NewJFrame class to call the system
 * 
 */
...
//                        String testDB = "SELECT * FROM `lhr_diagnosis` ORDER  BY `icd10_chapters`.`Id` ASC";
//                        ArrayList<ArrayList<String>> displaydb = null ;
//                        displaydb = rc.getQuerySQL(host, port, testDB);
//                        System.out.println(displaydb);
                        ...
                        */
                        ...
                        if ("ALL".equals(faculty)){ //no prepared statement in this loop for faculty == ALL
                            ...
                            sql = "SELECT COUNT(*) AS tot_by_fac "
...
//                        header_table.addCell(cell4);
                        ...
                        sql = "SELECT Diagnosis_Cd "
                                + "from lhr_diagnosis "
                                + "LEFT JOIN icd10_codes "
                                + "ON lhr_diagnosis.Diagnosis_Cd = icd10_codes.icd10_code "
                                + "WHERE icd10_code IS NULL "
                                + "AND centre_code = '"+ faculty +"' "
                                + "AND LENGTH(Person_Id_No) = '"+ patient_type +"' "
                                + "AND CAST(Episode_Date as DATE) BETWEEN '"+ date1 +"' AND '"+ date2 +"'";                            
                        ...
                        */
                        ...
                        for (i = 1; i <= chapter_map.size(); i++){ // chapter_map.size() is total of keys during .put                               
                              ...
                              if ("ALL".equals(faculty)){ //no prepared statement in this loop for faculty == ALL
                                  ...
                                  sql = "SELECT COUNT(Diagnosis_Cd) AS count "
                                          + "from lhr_diagnosis ld, icd10_codes ic "
                                          + "WHERE Diagnosis_Cd REGEXP '^[a-zA-Z0-9]+$'  "
                                          + "AND substring(Diagnosis_Cd,1,2) = '"+ String.format("%02d", i) +"' "
                                          + "AND ic.icd10_code = ld.Diagnosis_Cd "
                                          + "AND LENGTH(Person_Id_No) = '"+ patient_type +"' "
                                          + "AND CAST(Episode_Date as DATE) BETWEEN '"+ date1 +"' AND '"+ date2 +"' LIMIT 1";
                              ...
                              if (!"0".equals(chapter_total_result)){	// check chapter_total_result. if != 0 enter loop
                                  ...
                                  */
                                  ...
                                  if ("ALL".equals(faculty)){
                                      ...
                                      sql = "SELECT "
                                              + "Diagnosis_Cd, idc, id, name, M, F, total "
                                              + "FROM icd10_blocks, "
                                              ...
                                              + "AND LENGTH(Person_Id_No) = '"+ patient_type +"' "
                                              + "AND CAST(ld.Episode_Date as DATE) BETWEEN '"+ date1 +"' AND '"+ date2 +"' "
                                              + "group by substring(Diagnosis_Cd,3,3)) AS lolcat "
                                              + "WHERE id = diag AND idc = '"+ String.format("%02d", i) +"'";
                                      /*
                                     ...
                                     //jTextArea1.append("\n\t" + block_id_result + "   " + block_name_result + "   "+ block_total_result +"\n");
                                      ...
                                      if ("ALL".equals(faculty)){
                                          sql = "SELECT "
                                                  ...
                                                  + "WHERE Diagnosis_Cd REGEXP '^[a-zA-Z0-9]+$' "
                                                  + "AND substring(Diagnosis_Cd,3,3) ='"+ block_id_result +"' "
                                                  + "AND ib.Id = '"+ block_id_result +"' "
                                                  + "AND ic.icd10_code = ld.Diagnosis_Cd "
                                                  + "AND LENGTH(Person_Id_No) = '"+ patient_type +"' "
                                                  + "AND CAST(ld.Episode_Date as DATE) BETWEEN '"+ date1 +"' AND '"+ date2 +"' "
                        ...
                        if (invalid_record_list.size() != 0){
                        	document.add(new Phrase("\n"));
                        	document.add(new Phrase(invalid_record_list.size() + " invalid records founded in table `lhr_diagnosis`.`Diagnosis_Cd` : " + invalid_record_list));
                        }
